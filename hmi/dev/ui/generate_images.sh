...
# Cleanup
rm -f $SCRIPT_DIR/$VARIANT/*.png $SCRIPT_DIR/$VARIANT/*.jpg || true
mkdir -p $SCRIPT_DIR/$VARIANT
...
# 0: Background
cp $BACKGROUND $SCRIPT_DIR/$VARIANT/0.png
...
for i in {16..18} {21..31} 35 36 38 {41..45}; do
  cp ${SCRIPT_DIR}/pics/$i.png ${SCRIPT_DIR}/$VARIANT/$i.png
done

# 32: Splash screen (JPG!)
if [[ $VARIANT = eu* ]]; then
  cp ${SCRIPT_DIR}/pics/32.jpg ${SCRIPT_DIR}/$VARIANT/32.jpg
else
  cp ${SCRIPT_DIR}/pics/32-portrait.jpg ${SCRIPT_DIR}/$VARIANT/32.jpg
fi